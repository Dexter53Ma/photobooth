...
export function ContactForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const typeLabel = eventTypes[formData.typeEvenement] || formData.typeEvenement;

    const message = `Bonjour ! Je souhaite obtenir un devis.

📋 *Détails de la demande:*
• Type d'événement: ${sanitize(typeLabel) || "Non précisé"}
• Date souhaitée: ${sanitize(formData.date) || "À définir"}

👤 *Mes coordonnées:*
• Nom: ${sanitize(formData.nom)}
• Téléphone: ${sanitize(formData.telephone)}
    ...
    <main>
      ...
      <section className="bg-[#1E1E2A] py-3 px-4">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-center gap-4 md:gap-8">
          ...
          <a href="mailto:contact@marrakechphotobooths.com" className="flex items-center gap-2 font-suisse text-[0.875rem] max-md:text-[0.8125rem] text-white/90 hover:text-white transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-10 lg:gap-12">
            ...
            <div className="p-5 max-md:p-4 bg-[#F6F6F6] rounded-[1.5rem]">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3 max-md:gap-2.5">
                ...
                <button type="submit" className="w-full rounded-full bg-[#25D366] px-8 py-3 max-md:py-2.5 text-[max(14px,1.0625rem)] font-medium text-white transition-colors hover:bg-[#128C7E] font-suisse active:scale-[0.98] flex items-center justify-center gap-2">
